...
def _lerp_color(color_a, color_b, factor):
    factor = max(0.0, min(1.0, factor))
    return tuple(
        int(round(color_a[i] + (color_b[i] - color_a[i]) * factor))
        for i in range(3)
    )


def _resolve_dynamic_color(theme_data, value, key):
    stops = theme_data.get(key)
    if not stops or value is None:
        return None
    ...
    ordered_stops = sorted(stops, key=lambda stop: stop["value"])
    if not ordered_stops:
        return None

    if numeric_value <= ordered_stops[0]["value"]:
        return tuple(ordered_stops[0]["color"])

    for prev_stop, next_stop in zip(ordered_stops, ordered_stops[1:]):
        if numeric_value <= next_stop["value"]:
            span = next_stop["value"] - prev_stop["value"]
            factor = 0 if span == 0 else (numeric_value - prev_stop["value"]) / span
            return _lerp_color(tuple(prev_stop["color"]), tuple(next_stop["color"]), factor)

    return tuple(ordered_stops[-1]["color"])


def display_themed_value(theme_data, value, min_size=0, unit='', color_value=None):
    ...
    dynamic_value = value if color_value is None else color_value
    font_color = _resolve_dynamic_color(theme_data, dynamic_value, "FONT_COLOR_STOPS") or theme_data.get("FONT_COLOR", (0, 0, 0))
...
def display_themed_progress_bar(theme_data, value):
    ...
    bar_color = _resolve_dynamic_color(theme_data, value, "BAR_COLOR_STOPS") or theme_data.get("BAR_COLOR", (0, 0, 0))
...
def display_themed_radial_bar(theme_data, value, min_size=0, unit='', custom_text=None):
    ...
    bar_color = _resolve_dynamic_color(theme_data, value, "BAR_COLOR_STOPS") or theme_data.get("BAR_COLOR", (0, 0, 0))
    font_color = _resolve_dynamic_color(theme_data, value, "FONT_COLOR_STOPS") or theme_data.get("FONT_COLOR", (0, 0, 0))
        ...
        for custom_stat in config.THEME_DATA['STATS']['CUSTOM']:
            if custom_stat != "INTERVAL":
                ...
                if theme_data is not None and string_value is not None:
                    display_themed_value(theme_data=theme_data, value=string_value, color_value=numeric_value)
